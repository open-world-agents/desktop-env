...
IMPORTANT: Don't add any additional explanation - just respond with the word positioned lowest on screen"""
        ...
        self.frame_lock = threading.Lock()  # Thread lock for frame queue access

        self.stop_event = threading.Event()
        self.is_processing = threading.Event()  # Indicates if a frame is currently being processed
    ...
    def process_frame(self, frame: FrameStamped):
        ...
        # Skip processing if already handling a frame to prevent overload
        if self.is_processing.is_set():
            logger.debug("Skipping frame - currently processing another frame")
            return
        ...
        try:
            ...
            self.is_processing.clear()  # Reset processing flag
